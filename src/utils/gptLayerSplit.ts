...
// ── 透明背景版（測試用）────────────────────────────────────────────────────────
// 改用「移除背景，主體完全不動」的指令取代「提取主體」，降低 GPT 重繪幅度
async function extractOneLayerTransparent(
    obj: DetectedObject,
    ...
    try {
        onProgress?.(`🔍 透明去背：${obj.label}`);

        const isolated = await callAtlasImg2Img(
            `In this image, remove ONLY the background and make it fully transparent. ` +
            `Keep the "${obj.labelEn}" (${obj.label}) COMPLETELY UNCHANGED — ` +
            `same pixels, same colors, same textures, same lighting, same position and size. ` +
            `Do NOT redraw, reimagine, enhance, or alter the "${obj.labelEn}" in any way whatsoever. ` +
            `The subject must look identical to the original image. ` +
            `Only the background areas outside the "${obj.labelEn}" should become transparent.`,
            'gpt-image-2',
            atlasKey,
            compressedImage,
            1,
        ...
        console.warn(`[magicLayerTransparent] Skip "${obj.label}":`, e);
        return null;
    }
}

export async function gptLayerSegmentTransparent(
    imageBase64: string,
    geminiApiKey: string,
    atlasKey: string,
    falKey?: string,
    ...
    onProgress?.('🔍 Gemini 分析圖片語意與位置中...');
    const objects = await detectObjects(imageBase64, geminiApiKey);
    onProgress?.(`✨ 偵測到 ${objects.length} 個元素，GPT 透明背景模式，平行處理中...`);
    ...
    const bgPromise = callAtlasImg2Img(
        ...
        1,
        { ratio: detectedRatio },
    ).catch(e => { console.warn('[magicLayerTransparent] Background inpainting failed:', e); return null; });

    const objectResults = await Promise.all(
        objects.map((obj, i) => {
            onProgress?.(`🎨 透明提取第 ${i + 1}/${objects.length} 層：${obj.label}`);
            return extractOneLayerTransparent(obj, compressedImage, detectedRatio, atlasKey, onProgress);
        })
    );

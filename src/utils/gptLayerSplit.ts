...
// ── 智慧去背：偵測主體後 GPT 直接透明輸出 ───────────────────────────────────
/**
 * 偵測圖片中最重要的前景主體標籤（供 GPT 智慧去背使用）
 */
export async function detectPrimarySubject(
    imageBase64: string,
    apiKey: string,
): Promise<{ label: string; labelEn: string }> {
    const ai = new GoogleGenAI({ apiKey });
    const cleanBase64 = imageBase64.split(',')[1] || imageBase64;
    const mimeType = imageBase64.match(/data:(.*);base64/)?.[1] ?? 'image/png';
    ...
    const response = await ai.models.generateContent({
        ...
        contents: {
            parts: [
                { inlineData: { mimeType, data: cleanBase64 } },
                {
                    text: `What is the single most important foreground subject in this image?
Return ONLY a JSON object, no markdown:
{"label":"中文名稱","labelEn":"English name"}
Example: {"label":"人物","labelEn":"person"}`
                },
            ],
        },
    });

    const text = (response.text ?? '').replace(/```(?:json)?\s*/gi, '').replace(/```/g, '').trim();
    const match = text.match(/\{[\s\S]*?\}/);
    if (!match) throw new Error('Gemini 未能識別主體');
    const result = JSON.parse(match[0]);
    if (!result.labelEn) throw new Error('Gemini 回傳格式錯誤');

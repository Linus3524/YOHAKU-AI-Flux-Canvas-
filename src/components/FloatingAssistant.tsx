...
const FEATURE_DOCS = [
  ...
  },
  {
    ...
    items: [
      ...
      { title: "批次匯出（PNG）", desc: "在圖層面板同時選取 2 個以上工作區域時，可一鍵批次匯出全部為獨立 PNG 檔。" },
      { title: "匯出 SVG（向量圖層）", desc: "選取工作區域後，點設定面板「匯出工作區域（SVG）」，可選擇要匯出的工作區域，每個各自輸出為獨立 .svg 檔案。SVG 內每張圖片、文字、形狀皆為獨立物件，在 Adobe Illustrator 開啟後可個別選取與編輯。" }
    ]
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      { title: "擴展圖片 (Outpainting)", desc: "拖曳外框定義擴展區域，AI 將自動進行邊界生成與漸降 Poisson 拼接融合，100% 保留原圖像素無縫接合（支援自動分析並生成最佳擴圖提示詞）。" },
      ...
      { title: "一鍵跨平台適配", desc: "選擇任意圖像或設計稿，點選「一鍵跨平台適配」，可自動將其重新剪裁、排版並縮放生成為多種常用的社群與廣告比例尺寸（例如 1:1、16:9、9:16），自動適配多平台格式。" },
      { title: "延伸品牌套件", desc: "選取主標誌（Logo）圖片，輸入品牌名稱與標語（Slogan），AI 可一鍵延伸生成信封、名片、提袋、馬克杯等多種真實質感的品牌樣機（Mockup）展示。" },
      { title: "產品行銷組圖", desc: "電商商品專屬功能。選擇一張商品去背照，可一鍵生成適用於 Amazon、Shopify、Meta/Google 廣告等平台的成套行銷與情境套圖。支援「保持整組風格與色調一致」功能，自動調用 Gemini 快速預分析商品配色與光影特徵，並鎖定批次隨機種子碼（Seed）達到完美的品牌一致性。" }
    ]
  },
  {
...
const NAV_ITEMS: { id: Section; label: string; icon: React.ReactNode; purple?: boolean }[] = [
  ...
  },
  {
    id: 'local_models',
    label: '本機 AI 模型',
    icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round" style={{ display: 'block' }}><path d="M12 13V7"/><path d="m15 10-3 3-3-3"/><rect width="20" height="14" x="2" y="3" rx="2"/><path d="M12 17v4"/><path d="M8 21h8"/></svg>
  },
];
        ...
        >
          ...
          <div ref={contentRef} className="flex-1 overflow-y-auto bg-white/30" style={{ scrollbarWidth: 'thin', scrollbarColor: '#d1d1d1 transparent' }}>
            ...
            {activeSection === 'features' && (
              <div className="p-8">
                <h2 className="text-2xl font-bold text-yohaku-text-main mb-6" style={{ fontFamily: "'Noto Serif JP', serif" }}>六大核心特色</h2>
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-white/80 rounded-xl p-6 border border-gray-100 shadow-sm">
                    ...
                    <div className="grid grid-cols-2 gap-x-6 gap-y-4">
                      {[
                        ...
                        { t: '智慧對齊參考線', d: '右鍵空白畫布可開啟/關閉。拖曳物件時自動吸附鄰近物件的邊界與中心線，並顯示紅色輔助虛線。' },
                      ].map((item, i) => (
                        <div key={i}>
                  ...
                  <div className="bg-white/80 rounded-xl p-6 border border-emerald-100 shadow-sm">
                    ...
                    <div className="bg-purple-50/20 border border-purple-100/80 rounded-xl p-4 mb-3">
                      ...
                      <div className="grid grid-cols-2 gap-1.5 mb-2">
                        {[
                          ...
                          { label: '社群圖卡', desc: 'IG、小紅書等社群貼文專用排版卡片' },
                    ...
                    <div className="bg-purple-50/20 border border-purple-100/80 rounded-xl p-4 mb-3">
                      ...
                      <p className="text-[11px] text-gray-700 leading-relaxed mb-2">右鍵「魔法分層」，AI 自動識別人物、產品、文字、裝飾等語意元素進行分割並補全背景。支援「兩階段規劃與圖層清單預覽」，可於生成前自由刪除預計圖層。提供任務佇列與漸進式回貼機制；即夢負責逐物件隔離生成，再由軟體去背並依分析座標精確回貼。</p>
                      <div className="text-[11px] text-purple-900 bg-white/80 px-3 py-2 rounded-lg">✔ 支援即夢逐物件生成，並由軟體完成去背與圖層整理。需 <code className="font-mono bg-purple-50 px-1 rounded">Atlas Key</code>；未設定時降級為 Gemini 模式。</div>
                    </div>
                    ...
                    <div className="bg-blue-50/20 border border-blue-100/80 rounded-xl p-4 mb-3">
                      ...
                      <div className="grid grid-cols-1 gap-2 mb-2.5">
                        {[
                          { label: '產品行銷組圖', desc: '選擇去背產品圖，一鍵生成成套電商行銷物料與情境套圖（Amazon/Shopify/Meta 廣告等）。支援「保持整組風格與色調一致」功能，自動由 Gemini 提取商品配色與光影特測，並鎖定批次隨機 Seed，達到完美的品牌統一感。' },
                          ...
                          <div key={i} className="bg-white/80 border border-blue-50 rounded-xl p-3 shadow-[0_1px_3px_rgba(0,0,0,0.02)]">
                            <div className="font-bold text-blue-700 text-[12px] mb-1">{m.label}</div>
                  ...
                  <div className="bg-white/80 rounded-xl p-6 border border-indigo-100 shadow-sm">
                    ...
                    <div className="grid grid-cols-2 gap-x-6 gap-y-4 mb-4">
                      {[
                        ...
                        { t: '即夢圖層分離', d: '在工作流中可串接「圖層分離 (Layer Split)」節點，由 Gemini 分析物件、即夢逐層生成，再由軟體去背並整理成可編輯圖層。' },
                        ...
                        <div className="overflow-x-auto">
                          <table className="w-full text-[10px] text-gray-600 border-collapse">
                            ...
                            <tbody className="divide-y divide-gray-100">
                              {[
                                ['底層架構', 'gemini-3.1-flash-lite-image', 'gemini-3.1-flash-image', 'gemini-3-pro-image'],
                        ...
                        <div className="overflow-x-auto">
                          <table className="w-full text-[10px] text-gray-600 border-collapse">
                            ...
                            <tbody className="divide-y divide-gray-100">
                              ...
                              <tr>
                                <td className="py-2 pr-2 font-bold text-gray-700 align-top">通義千問 Qwen Image 2.0</td>
                                <td className="py-2 pr-2 text-gray-500 align-top">Alibaba</td>
                                <td className="py-2 text-gray-600 align-top">阿里巴巴頂級多模態生圖模型，中文理解能力出色，支援中英文提示詞（最多 800 字），適合電商、設計素材與創意圖片生成。支援<span className="font-bold text-orange-600">圖生圖</span>。</td>
                              </tr>
                            </tbody>
                          </table>
                        </div>

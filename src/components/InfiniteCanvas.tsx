...
import { ElementsLayer } from './CanvasElementsLayer';
...
const SelectionMenuIcons = {
    MagicFilled: () => <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" style={{ display: 'block' }}><path d="M9.813 15.904 9 18.75l-.813-2.846a4.5 4.5 0 0 0-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 0 0 3.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 0 0 3.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 0 0-3.09 3.09ZM18.259 8.715 18 9.75l-.259-1.035a3.375 3.375 0 0 0-2.455-2.456L14.25 6l1.036-.259a3.375 3.375 0 0 0 2.455-2.456L18 2.25l.259 1.035a3.375 3.375 0 0 0 2.456 2.456L21.75 6l-1.035.259a3.375 3.375 0 0 0-2.456 2.456ZM16.894 20.567 16.5 21.75l-.394-1.183a2.25 2.25 0 0 0-1.423-1.423L13.5 18.75l1.183-.394a2.25 2.25 0 0 0 1.423-1.423l.394-1.183.394 1.183a2.25 2.25 0 0 0 1.423 1.423l1.183.394-1.183.394a2.25 2.25 0 0 0-1.423 1.423Z"/></svg>,
      ...
      >
        ...
        {(() => {
          const generatingEls = generatingElementIds
            ...
            .filter((el): el is NonNullable<typeof el> => !!el && el.type !== 'artboard');
          // 多張同時運算時，badge 只顯示在面積最大的元素上
          const badgeId = generatingEls.length <= 1
            ? generatingEls[0]?.id
            : generatingEls.reduce((max, el) =>
                el.width * el.height > max.width * max.height ? el : max
              ).id;
          return generatingEls.map(el => (
            <div
              key={`shimmer-${el.id}`}
                                            ...
                                            >
                                                ...
                                                {(() => {
                                                  const CATS = [
                                                    ...
                                                    { label: '動漫與漫畫',     ids: ['Comic Book','Japanese Anime','Manga Ink','Chibi','Webtoon','Mecha','Retro Cel Anime','Radiant Shinkai','Soft KyoAni','City Pop Graphic','Copic Manga'] },
                                                  ...
                                                  const byId = Object.fromEntries(stylePresets.map(s => [s.id, s]));
                                                  return CATS.map(cat => (
                                                    <optgroup key={cat.label} label={cat.label}>
                                                      {cat.ids.map(id => {
                                                        const s = byId[id];
                                                        return s ? <option key={s.id} value={s.label}>{s.name}</option> : null;
                                                      })}
                                                    </optgroup>
                                                  ));
                                                })()}
                                    ...
                                    <div className="border-t border-gray-100">
                                      ...
                                      {showAdvanced && (
                                        <div className="px-5 pb-4 flex flex-col gap-3">
                                          ...
                                          {/* 自訂風格種子 (Seed) - 如果是單純便利貼，已移至生成設定，這裡隱藏 */}
                                          {(!hasNote || hasImageOrDrawingOrShape) && (
                                              <div className="flex flex-col gap-1.5">
                                                  <div className="flex items-center justify-between">
                                                      <span className="text-[11px] font-medium text-gray-500">自訂風格種子 (Seed)</span>
                                                      <div

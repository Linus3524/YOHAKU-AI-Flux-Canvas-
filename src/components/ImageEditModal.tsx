...
const compositeImagesPixelPerfect = async (originalSrc: string, generatedSrc: string, bwMaskSrc: string): Promise<string> => {
  ...
  for (let i = 0; i < maskData.data.length; i += 4) {
    // Use the red channel of the blurred mask as an alpha blend factor (0.0 to 1.0)
    // Boost the alpha slightly to ensure the core of the mask remains fully opaque
...
const createPrefilledImage = async (baseSrc: string, maskDataUrl: string): Promise<string> => {
  const loadImage = (src: string): Promise<HTMLImageElement> => new Promise((resolve, reject) => {
    const img = new Image();
    img.crossOrigin = "anonymous";
    img.onload = () => resolve(img);
  ...
  for (let i = 0; i < maskData.data.length; i += 4) {
    // If painted in mask, blend with neutral gray

...
export const LayerPanel: React.FC<LayerPanelProps> = ({ 
    ...
    // Reverse elements for layer list (top z-index first)
    ...
    // Drag and Drop Handlers (for sorting layers)
    ...
    return (
        <div 
            ref={panelRef}
            style={{ left: position.x, top: position.y }}
            className={`fixed z-[998] w-64 bg-white/80 backdrop-blur-xl rounded-2xl shadow-[0_20px_40px_rgba(0,0,0,0.12)] border border-white/50 flex flex-col overflow-hidden transition-shadow animate-fade-in-right font-sans ${isDragging ? 'shadow-2xl' : 'shadow-[0_20px_40px_rgba(0,0,0,0.12)]'}`}
        >
            {/* Header (Drag Handle) */}
            <div 
                ...
                {sortedElements.map(el => {
                    ...
                    return (
                        <div 
                            ...
                            className={`
                                group flex items-center gap-2 p-2 rounded-lg text-sm border-2 transition-all select-none
                        ...
                        >
                            ...
                            {/* Name */}
                            <div className="flex-1 truncate font-medium text-[#1D1D1F]" onDoubleClick={(e) => { e.stopPropagation(); handleStartRename(el.id, el.name); }}>
                                {editingId === el.id ? (
                                    <input 
                                        autoFocus
                                        value={editName}
                                    ...
                                    <span className="flex items-center gap-2">
                                        {el.type === 'text' && (el as any).text ? `T: ${(el as any).text.substring(0, 10)}...` : el.name}
            ...
            {/* Footer Status */}
            <div className="px-4 py-2 border-t border-black/5 bg-[#F5F5F7] text-[10px] text-[#86868B] text-center cursor-default">
                {elements.length} 個圖層
            </div>
        </div>
    );
